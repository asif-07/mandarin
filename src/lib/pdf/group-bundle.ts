...
const GROUP_SELECT =
  "id, travel_date, travel_end_date, group_code, label, guide_name, reference_prefix, entry_port, exit_port, source, partner_code, pax_expected, pack_path, pack_uploaded_at, package_tier, hotel_name, visa_status, visa_applied_at, visa_path, visa_uploaded_at, group_documents(id, doc_type, file_name, storage_path, mime_type, uploaded_at, deleted_at), travellers(id, traveller_ref, full_name, passport_number, nationality, travel_start_date, travel_end_date, visa_reference, status, updated_at, traveller_documents(id, doc_type, file_name, storage_path, mime_type, merge_order, uploaded_at, deleted_at))";
...
function cacheKey(group: GroupRow, logo: BundleLogo, partnerLogoPath: string | null): string {
  ...
  const cover = [group.group_code, group.label, group.guide_name, group.reference_prefix, group.entry_port, group.exit_port, group.travel_date, group.travel_end_date, group.partner_code, group.pax_expected, group.package_tier, group.hotel_name, group.visa_applied_at];
