...
import { BUCKETS, CHINA_PORTS, PACKAGE_TIERS, tierHasHotel, tierHasTransit, tierNeedsStars } from "@/lib/constants";
...
type Form = { code: string; entry_port: string; exit_port: string; label: string; guide_name: string; notes: string; package_tier: string | null; hotel_name: string; hotel_stars: number | null; transit_location: string | null };
const EMPTY: Form = { code: "", entry_port: "", exit_port: "", label: "", guide_name: "", notes: "", package_tier: null, hotel_name: "", hotel_stars: null, transit_location: null };
...
export function B2bUploadButton({ variant = "default" }: { variant?: "default" | "outline" }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      ...
      setUploading(false);
      const res = await registerB2bGroup({ code: form.code, upload_path: path, file_name: file.name, entry_port: form.entry_port, exit_port: form.exit_port, label: form.label, guide_name: form.guide_name, notes: form.notes, package_tier: form.package_tier, hotel_name: form.hotel_name, hotel_stars: form.hotel_stars, transit_location: form.transit_location });
        ...
        <DialogContent className="sm:max-w-xl">
          ...
          <fieldset disabled={busy} className="grid gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {tierHasTransit(form.package_tier) && (
                <div className="space-y-1.5">
                  <Label htmlFor="b2b_transit">Transit location (optional)</Label>
                  <TransitSelect id="b2b_transit" value={form.transit_location} onChange={(v) => setForm({ ...form, transit_location: v })} />
                </div>
              )}
